...
fun OnboardingArt(scene: OnbScene, modifier: Modifier = Modifier) {
    ...
    Canvas(modifier) {
        drawBackdrop(pal, pulse)
        when (scene) {
            OnbScene.DISCOVER -> drawDiscover(pal, spin, pulse)
            OnbScene.UNLOCK -> drawUnlock(pal, drift, pulse)
            OnbScene.TENDER -> drawTender(pal, pulse)
            OnbScene.POST -> drawPost(pal, drift, pulse)
            OnbScene.RECEIVE -> drawReceive(pal, drift, pulse)
            OnbScene.COMPARE -> drawCompare(pal, pulse)
            OnbScene.AWARD -> drawAward(pal, pulse)
        }
    }
}
...
private fun DrawScope.drawBackdrop(pal: BhqColors, pulse: Float) {
    val glow = size.minDimension * 0.56f
    drawCircle(
        brush = Brush.radialGradient(
            listOf(pal.accent.copy(alpha = 0.14f + 0.06f * pulse), Color.Transparent),
            center = center, radius = glow,
        ),
        radius = glow, center = center,
    )
    val step = size.minDimension / 7f
    val line = pal.blueprintLine.copy(alpha = 0.06f)
    var x = step
...
private fun DrawScope.drawDiscover(pal: BhqColors, spin: Float, pulse: Float) {
    val c = center
    val maxR = size.minDimension * 0.42f
    listOf(0.4f, 0.7f, 1f).forEach { f ->
        drawCircle(pal.blueprintLine.copy(alpha = 0.18f), maxR * f, c, style = Stroke(1.2.dp.toPx()))
    }
    drawLine(pal.blueprintLine.copy(0.12f), Offset(c.x - maxR, c.y), Offset(c.x + maxR, c.y), 1f)
    drawLine(pal.blueprintLine.copy(0.12f), Offset(c.x, c.y - maxR), Offset(c.x, c.y + maxR), 1f)
    rotate(spin, pivot = c) {
        drawArc(
            brush = Brush.sweepGradient(listOf(Color.Transparent, pal.accent.copy(alpha = 0.5f)), c),
    ...
    nodes.forEachIndexed { i, (rf, deg) ->
        ...
        drawCircle(pal.accent.copy(0.22f), s * 2.4f, p)
        drawCircle(pal.accentLight, s, p)
    }
}

private fun DrawScope.drawUnlock(pal: BhqColors, drift: Float, pulse: Float) {
    val c = center
    ...
    drawArc(
        color = pal.accentLight, startAngle = 180f, sweepAngle = 180f, useCenter = false,
        topLeft = Offset(c.x - lockW * 0.28f, lockTop - lockH * 0.55f - lift),
        size = Size(lockW * 0.56f, lockH * 0.8f),
        style = Stroke(4.dp.toPx(), cap = StrokeCap.Round),
    )
    drawRoundRect(
        brush = Brush.verticalGradient(listOf(pal.accent, DeepTeal)),
    ...
    repeat(3) { i ->
        ...
        drawRoundRect(pal.blueprintLine.copy(0.10f), Offset(rl, ry), Size(rowW, rowH), CornerRadius(rowH / 2), style = Stroke(1f))
        drawCircle(pal.accentLight.copy(0.85f), rowH * 0.20f, Offset(rl + rowH * 0.62f, ry + rowH / 2))
    ...
    drawRect(
        brush = Brush.verticalGradient(
            listOf(Color.Transparent, pal.accentLight.copy(0.20f), Color.Transparent),
...
private fun DrawScope.drawTender(pal: BhqColors, pulse: Float) {
    val c = center
    val unit = size.minDimension
    ...
    heights.forEachIndexed { i, h ->
        ...
        drawRoundRect(
            brush = if (tallest) Brush.verticalGradient(listOf(pal.accentLight, pal.accent)) else SolidColor(pal.blueprintLine.copy(0.30f)),
            topLeft = Offset(x, baseY - bh), size = Size(barW, bh),
            cornerRadius = CornerRadius(barW * 0.4f),
        )
        if (tallest) drawSeal(pal, Offset(x + barW / 2, baseY - bh - unit * 0.10f), unit * 0.06f * (1f + 0.06f * pulse))
        x += barW + gap
    }
    drawLine(
        pal.blueprintLine.copy(0.18f),
...
private fun DrawScope.drawPost(pal: BhqColors, drift: Float, pulse: Float) {
    val c = center
    val unit = size.minDimension
    val docW = unit * 0.46f
    val docH = unit * 0.56f
    ...
    drawRoundRect(pal.blueprintLine.copy(0.16f), Offset(left, top), Size(docW, docH), CornerRadius(unit * 0.04f), style = Stroke(1.2.dp.toPx()))
    ...
    repeat(5) { i ->
        ...
        drawRoundRect(pal.accent.copy(0.55f), Offset(lineX, ly), Size(w, lineH), CornerRadius(lineH / 2))
    }
    val bc = Offset(left + docW * 0.92f, top + docH * 0.9f)
    val br = unit * 0.10f * (1f + 0.05f * pulse)
    drawCircle(Brush.verticalGradient(listOf(pal.accentLight, pal.accent)), br, bc)
    val a = Path().apply {
...
private fun DrawScope.drawReceive(pal: BhqColors, drift: Float, pulse: Float) {
    val c = center
    val unit = size.minDimension
    val hubR = unit * 0.13f * (1f + 0.05f * pulse)
    drawCircle(Brush.radialGradient(listOf(pal.accent.copy(0.45f), Color.Transparent), c, hubR * 2.3f), hubR * 2.3f, c)
    drawCircle(Brush.verticalGradient(listOf(pal.accentLight, pal.accent)), hubR, c)
    val starts = listOf(
    ...
    starts.forEachIndexed { i, s0 ->
        ...
        val cw = unit * 0.15f
        val ch = unit * 0.10f
        drawLine(pal.accent.copy(0.10f), s0, c, 1f)
        drawRoundRect(Color.White.copy(alpha * 0.12f), Offset(pos.x - cw / 2, pos.y - ch / 2), Size(cw, ch), CornerRadius(unit * 0.02f))
        drawRoundRect(pal.blueprintLine.copy(alpha * 0.4f), Offset(pos.x - cw / 2, pos.y - ch / 2), Size(cw, ch), CornerRadius(unit * 0.02f), style = Stroke(1f))
    }
}

private fun DrawScope.drawCompare(pal: BhqColors, pulse: Float) {
    val c = center
    val unit = size.minDimension
    ...
    data.forEach { (h, win) ->
        val bh = unit * h
        drawRoundRect(
            brush = if (win) Brush.verticalGradient(listOf(pal.accentLight, pal.accent)) else SolidColor(pal.blueprintLine.copy(0.28f)),
            topLeft = Offset(x, baseY - bh), size = Size(barW, bh), cornerRadius = CornerRadius(barW * 0.18f),
        )
        if (win) drawSeal(pal, Offset(x + barW / 2, baseY - bh - unit * 0.09f), unit * 0.055f * (1f + 0.06f * pulse))
        x += barW + gap
    }
    drawLine(
        pal.blueprintLine.copy(0.18f),
...
private fun DrawScope.drawAward(pal: BhqColors, pulse: Float) {
    val c = center
    val unit = size.minDimension
    val rays = 12
    ...
    repeat(rays) { i ->
        ...
        drawLine(pal.accent.copy(0.5f), p1, p2, 2.dp.toPx(), cap = StrokeCap.Round)
    }
    drawCircle(Brush.radialGradient(listOf(pal.accent.copy(0.4f), Color.Transparent), c, inner * 2.4f), inner * 2.4f, c)
    drawCircle(Brush.verticalGradient(listOf(pal.accentLight, pal.accent)), inner, c)
    drawSealCheck(c, inner)
}

// ── Shared seal ─────────────────────────────────────────────────────

private fun DrawScope.drawSeal(pal: BhqColors, at: Offset, r: Float) {
    drawCircle(pal.accent.copy(0.28f), r * 1.5f, at)
    drawCircle(pal.accent, r, at)
    drawSealCheck(at, r)
}

...
export const PARTNERS: Partner[] = [
  ...
  },
  {
    ...
    why: "Most homes are designed first and assessed for energy performance afterwards, which is precisely the wrong order: by the time a rating comes back, the orientation, the glazing and the shading are already settled. Neighbourhood Architecture inverts that. NatHERS approved energy modelling runs from the beginning of the design process, so orientation, glazing, shading, insulation and thermal performance can be tested while changes are still easy to make. The result is architecture that holds its design quality while cutting the heating and cooling a Canberra winter would otherwise demand.\n\nBehind it sits an uncommon pairing. Chris Clode is a registered architect; Brendan McGregor is an HIA GreenSmart Professional who led design at Light House Architecture and Science before the two founded the practice together. An owner therefore gets registered architectural oversight and genuine energy expertise from one small team, and can engage that team for the whole journey, from concept design and approvals through coordinated construction documentation, builder tendering and support during construction. Sixteen Master Builders and HIA wins and finalist honours since 2022 suggest the industry agrees, most recently the 2025 Master Builders ACT award for Sustainable Residential Project, for Tocumwal Revival. For an owner who wants a home that is beautiful, buildable and genuinely cheap to run, this is a rare combination.",
    about:
